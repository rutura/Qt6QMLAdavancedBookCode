...
Page {
    ...
    // Alias to proxy model for convenience
    property ContactFilterProxyModel proxyModel: contactManager.proxyModel
     ...
     header: ToolBar {
         ...
         RowLayout {
             ...
             Text {
                 text: root.proxyModel.count + " contacts"
                 font.pixelSize: 14
                 color: "#6B7280"
             }
             ...
             Rectangle {
                 ...
                 ListView {
                     ...
                     delegate: ContactDelegate {
                         ...
                         onClicked: root.contactSelected(root.proxyModel.mapToSourceIndex(index))

                         onFavoriteToggled: {
                            root.contactManager.toggleFavorite(root.proxyModel.mapToSourceIndex(index))
                         }

                         // Swipe right to edit
                         onEditRequested: {
                             root.editContactRequested(root.proxyModel.mapToSourceIndex(index))
                         }

                         // Swipe left to delete
                         onDeleteRequested: {
                             root.deleteContactRequested(root.proxyModel.mapToSourceIndex(index), model.firstName, model.lastName, model.avatarColor)
                         }
