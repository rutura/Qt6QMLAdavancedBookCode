...
ContactManager::ContactManager(QObject *parent)
    ...
    , m_dataFilePath(JsonSerializer::getDefaultFilePath())
{
    ...
    // Load contacts from file, or generate sample data if no file exists
    loadContacts();
}
 ...
 * If no file exists, generates sample contacts.
 */
void ContactManager::loadContacts()
{
    QVector<Contact> contacts;

    if (JsonSerializer::fileExists(m_dataFilePath)) {
        if (JsonSerializer::loadContacts(m_dataFilePath, contacts)) {
            m_contactModel->addContacts(contacts);
            qDebug() << "ContactManager: Loaded" << contacts.size() << "contacts from file";
        } else {
            qWarning() << "ContactManager: Failed to load contacts:" << JsonSerializer::lastError();
            // Generate sample contacts as fallback
            generateSampleContacts(45);
        }
    } else {
        qDebug() << "ContactManager: No contacts file found, generating sample data";
        generateSampleContacts(45);
    }
}
...
{
    QVector<Contact> contacts = m_contactModel->getAllContacts();

    if (!JsonSerializer::saveContacts(contacts, m_dataFilePath)) {
        qWarning() << "ContactManager: Failed to save contacts:" << JsonSerializer::lastError();
    }
}
