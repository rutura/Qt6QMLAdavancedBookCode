...
public:
    ...
    bool hasMore() const { return m_issues.size() < m_totalCount; }
...
private slots:
    void onSearchResultsReady(const QList<Issue*> &issues, int page, int totalCount);
...
private:
    ...
    void appendBatch(const QList<Issue*> &batch);
    void resetWith(const QList<Issue*> &batch);
